...
def weapons_parser(state: SrdParserState) -> dict:
	...
	if Weapon.is_empty():
		...
		for item in response:
			...
			weapon_properties = session.scalars(select(WeaponProperty).where(WeaponProperty.name.in_(item['properties']))).all()
			weapon = Weapon(name=item['name'], damage=item['damage'], damage_type_id=damage_type.id, properties=weapon_properties, mastery_property_id=weapon_mastery_property.id, weight=item['weight'], cost=item['cost'])
			session.add(weapon)
		session.commit()
