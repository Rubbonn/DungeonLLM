...
def weapons_parser(state: SrdParserState) -> dict:
	...
	# Weapons extraction
	from app.entities.items import Weapon, WeaponProperty
	if Weapon.is_empty():
	...
	for animal in response.items:
		a = Animal(
			creature=Creature(
				...
				traits=[CreatureTrait(name=name, description=description) for name, description in animal.traits.items()]
			),
